...
exports.users = function(request, response) {
  ...
  if (request.user_role === 'admin') {
    // Fetch user data from the database for admin role
    db.all("SELECT userid, password FROM users", function(err, rows) {
      if (err) {
        // Handle any potential database error
        response.writeHead(500, { 'Content-Type': 'text/html' });
        response.end("<h1>Database error occurred.</h1>");
        return;
      }
      send_users(request, response, rows); // Send the user data back
    });
  } else {
    // For non-admin users, send a 403 Forbidden response with the error message on the page
    response.writeHead(403, { 'Content-Type': 'text/html' });
    response.end("<h1>Error: Admin Privileges Required To See Users</h1>");
  }
};
...
exports.find = function(request, response) {
  ...
  if (urlObj.query['title']) {
    ...
    let songs = urlObj.query['title'].split(' ')
    let songWords = '%' + songs.join('%') + '%'
  
    sql = `SELECT id, title FROM songs WHERE title LIKE '${songWords}'`;
